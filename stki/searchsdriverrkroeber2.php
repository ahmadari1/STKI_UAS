...
echo "<br /><br><table><tr><td></td><td><form action='searchsdriverrkroeber2.php' method='GET'><input type='text' onclick=value='' size='50' name='keyword' value='$keyword' style='width: 500px; height: 30px; font-size: 16px;'> <input type='submit' value='Search'></form></td></tr></table>";
...
<td>Url</td>
   ...
   $query = "SELECT * FROM (SELECT g.id id,kata, A, jmlkatakunci, B, jmlkatadok, jmlkatadok-A C,  A/2*(1/A+B+1/A+(jmlkatadok-A)) driverrkroeber FROM (SELECT *,jmlkatakunci-A B FROM (SELECT y.id,y.kata,y.freq A FROM (SELECT * FROM katakunci WHERE id='0') AS x JOIN (SELECT * FROM token WHERE id!='0' ORDER BY id+0) as Y ON x.kata=y.kata) AS d JOIN (SELECT COUNT(kata) jmlkatakunci FROM katakunci) AS e) AS f JOIN (SELECT id,COUNT(kata) jmlkatadok FROM token GROUP BY id) AS g ON f.id=g.id GROUP BY g.id ORDER BY driverrkroeber DESC) AS r JOIN (SELECT * FROM berita) AS s ON r.id=s.id";
   ...
   while ($row = mysqli_fetch_array($result)) {
      ...
      echo "<td><font color=blue></font>" . " $row[driverrkroeber] " . "<br></td>";
